...
export function MultiplayerPanel({ open, runId, state, selectedSystem, findings, onClose, onChanged }: MultiplayerPanelProps) {
  ...
  const changeClaim = async () => {
    if (!runId || !selectedSystem) return;
  ...
  return <div className="federation-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <section className="federation-panel" role="dialog" aria-modal="true" aria-labelledby="federation-title">
      <header className="federation-head">
        ...
        <div><span className="eyebrow">COOPERATIVE RESEARCH NETWORK</span><h2 id="federation-title">Research federation</h2><p>Divide the problem into territories. Share conclusions, not private workspaces.</p></div>
      ...
      {!state.enabled ? <div className="federation-entry">
        ...
        <div className={`federation-state ${state.session?.status ?? "paused"}`}><span><i />{state.session?.status === "active" ? "Federation online" : state.session?.status === "closed" ? "Host closed session" : "Host unreachable — session paused"}</span><small>{state.session?.mode === "host" ? "YOU ARE HOST" : "JOINED THROUGH TAILSCALE"}</small></div>
        <div className="federation-grid">
          ...
          <section aria-labelledby="territory-title"><div className="federation-section-head"><span>ACTIVE FRONT</span><h3 id="territory-title">Territory and discoveries</h3></div>
            <div className="territory-order"><small>SELECTED SYSTEM</small><strong>{selectedSystem?.title ?? "Select a system on the map"}</strong>{selectedSystem && <><span>{claimOwner ? `Claimed by ${claimOwner.display_name}` : "Unclaimed direction"}</span><button className="primary-button" onClick={() => void changeClaim()} disabled={Boolean(busy) || Boolean(selectedClaim && selectedClaim.player_id !== localPlayer?.id)}>{selectedClaim?.player_id === localPlayer?.id ? "Release system" : selectedClaim ? "Occupied" : "Claim system"}</button></>}</div>
            <div className="publish-order"><label>Publish a local finding<select value={findingId} onChange={(event) => setFindingId(event.target.value)}><option value="">Choose a finding</option>{findings.map((finding) => <option key={finding.id} value={finding.id}>{String(finding.title ?? finding.statement ?? finding.id)}</option>)}</select></label><button className="quiet-button" disabled={!findingId || Boolean(busy)} onClick={() => void publish()}>Publish summary</button><small>Only the bounded title and summary cross the tailnet.</small></div>
        ...
        <footer className="federation-foot"><span>Project data remains on participant devices. Netlify stores nothing.</span><button className="danger-button" onClick={() => void leave()} disabled={Boolean(busy)}>{state.session?.mode === "host" ? state.session.status === "closed" ? "Reset to single player" : "Close federation" : "Leave session"}</button></footer>
      </div>}
